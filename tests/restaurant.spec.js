...
describe('9 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {    
    ...
    assert.deepStrictEqual(Object.keys(objetoRetornado1.fetchMenu()), ['food', 'drink']);
    // ```
    // TESTE 3: Verifique que o menu passado pra função createMenu é identico ao menu recuperado pela função 'objetoRetornado.fetchMenu'
    // ```
    const objetoRetornado2 = createMenu({food: {}, drink: {}});
    // objetoRetornado.fetchMenu() // Retorno: objetoQualquer
    assert.deepStrictEqual({food: {}, drink: {}}, objetoRetornado2.fetchMenu());
    // ```
    ...
    assert.strictEqual(objetoRetornado6.pay(), 12.1);
    // ```
