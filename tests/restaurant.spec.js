...
describe('9 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    const testMenu2 = { food: { coxinha: 3, sopa: 7, sashimi: 30 }, drink: { agua: 2 } };
    const objetoRetornado = createMenu(testMenu);
    const objetoRetornado2 = createMenu(testMenu2);
    assert.deepStrictEqual(Object.keys(objetoRetornado.fetchMenu()), ['food', 'drink']);
    // TESTE 3: Verifique que o menu passado pra função createMenu é identico ao menu recuperado pela função 'objetoRetornado.fetchMenu'
    assert.deepStrictEqual(objetoRetornado2.fetchMenu(), testMenu2);
    ...
    assert.strictEqual(objetoRetornado2.pay(), 50);
