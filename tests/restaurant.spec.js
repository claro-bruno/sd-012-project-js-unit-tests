...
describe('9 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    let menu = { food: {}, drink: {} };
    assert.ok(createMenu(), { fetchMenu: () => {} });
    assert.ok(createMenu(menu), createMenu(menu).fetchMenu());
    ...
    assert.deepStrictEqual(createMenu(menu).fetchMenu(), { food: {}, drink: {} });
    ...
    objetoRetornado.order('sopa');
    objetoRetornado.order('sashimi');
    assert.ok(objetoRetornado.consumption, ['coxinha', 'agua', 'sopa', 'sashimi']);
    ...
    let newMenu = { food: {}, drink: {} };
    const newTest7Obj = createMenu(newMenu);
    newTest7Obj.order('coxinha');
    newTest7Obj.order('agua');
    newTest7Obj.order('coxinha');
    assert.deepStrictEqual(newTest7Obj.consumption, ['coxinha', 'agua', 'coxinha']);
    ...
    const conta = createMenu({ food: { coxinha: 4.00 }, drink: { agua: 2.00 } });
    conta.order('coxinha');
    conta.order('agua');
    conta.order('coxinha');
