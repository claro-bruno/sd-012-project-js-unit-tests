...
describe('9 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    // ```
    assert.notDeepStrictEqual(Object.keys(createMenu({})).length, 1)
    ...
    // ```
    assert.deepStrictEqual(createMenu({ comida: {}, bebida: {} }).fetchMenu(), { comida: {}, bebida: {} });
    ...
    // ```
    assert.deepStrictEqual(createMenu({ comida: {}, bebida: {} }).consumption, []);
    ...
    // ```
    const objetoRetornado = createMenu({ food: {'coxinha': 3.9, 'sopa': 9.9, 'sashimi': 20}, drink: {'agua': 3.9, 'cerveja': 6.9} });
    objetoRetornado.order('coxinha');
    ...
    // ```
    assert.deepStrictEqual(objetoRetornado.pay(), 41.6);
