...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  it('Verifica se ao receber um raio, a função `circle` retorna um objeto contedos os valores esperados', () => {
    ...
    const resultOf3 = () => {
      ...
      let obj = {};
      Object.keys(result).forEach((el) => obj[el] = parseFloat(result[el].toPrecision(4)));
      console.log(obj.area === 28.26)
      return obj;
    }
    assert.deepStrictEqual(resultOf3(), {area: 28.26, circumference: 18.84, radius: 3});
  });
});
