...
const addConsumption = (string) => restaurant.consumption.push(string);

const checkMenu = (item) => {
  const foods = restaurant.fetchMenu().food;
  const drinks = restaurant.fetchMenu().drink;
  const arrayFood = Object.keys(foods);
  return arrayFood.includes(item) ? foods[item] : drinks[item];
};
...
const createMenu = (obj) => {
  ...
  fetchMenu: () => obj,
  consumption: [],
  order: (string) => addConsumption(string),
  pay: () => calcTotal(),
  };
  return restaurant;
};
