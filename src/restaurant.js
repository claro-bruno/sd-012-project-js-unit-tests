...
const createMenu = (items) => ({
  ...
  order,
  pay() {
 return Number(this.consumption.reduce((accumulator, item) => {
    // console.log(myMenu.food[item]);
    if (this.fetchMenu().food[item]) {
      return accumulator + this.fetchMenu().food[item];
    }
    if (this.fetchMenu().drink[item]) {
      return accumulator + this.fetchMenu().drink[item];
    }
    return accumulator;
  }, 0).toFixed(2)); 
},
...
const myMenu = meuRestaurante.fetchMenu();

meuRestaurante.order('coxinha');
meuRestaurante.order('cerveja');
meuRestaurante.order('sopa');

const myConsumption = meuRestaurante.consumption; // Retorno: ['coxinha']
