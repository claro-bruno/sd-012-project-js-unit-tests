...
const payFunction = () => {
 ...
 const consuma = meuRestaurante.consumption;
 let conta = 0;
 for (let index = 0; index < consuma.length; index += 1) {
   if (menu.food[consuma[index]] !== undefined) {
     conta += menu.food[consuma[index]];
   }
   if (menu.drink[consuma[index]] !== undefined) {
     conta += menu.drink[consuma[index]];
   }
 }
 return 1.1 * conta;
};
